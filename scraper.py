...
def remove_unwanted_text(content, word):
    ...
    unwanted_patterns = [
        "» 類語の一覧を見る",
        f"[続きの解説]", f"({word} から転送)", "[可能]",
        f"「{word}」の発音・読み方", f"「{word}」の類語、言い換え表現",
        f"「{word}」の熟語・言い回し", f"「{word}」の定義を英語で解説",
        f"「{word}」の類語", f"「{word}」に関連する用語・表現",
        f"「{word}」とは・「{word}」の詳しい解説", f"「{word}」の語源・由来",
        f"「{word}」の類義語", f"「{word}」に似た言葉",
        "出典:", "[用法]", "[補説]", "[下接語] "
    ]
    ...
    content = re.sub(fr"^「{word}」とは、", "", content).replace(" ", " ")
...
def convert_word_to_hiragana(word):
    ...
    katakana_to_hiragana = {chr(k): chr(k - 96) for k in range(12450, 12534)}  # Katakana to Hiragana
...
def scrape_weblio(word, desired_reading=None):
    ...
    for h2_midashigo in h2_midashigos:
        ...
        looper = element_to_loop_over.next_siblings if element_to_loop_over.name == "br" else div_after_h2.contents
        ...
        if "日本語例文用例辞書はプログラムで機械的に例文を生成しているため、不適切な項目が含まれていることもあります。ご了承くださいませ" in final_content:
            continue
    
        final_content = remove_unwanted_text(final_content, word)
        final_content = re.sub(fr"^「{word}」とは、", "", final_content)
        final_content = final_content.replace(" ", " ")
        # 発音・読み方
        if yomikata != "null" and len(final_content) < 400:
            yomikata = get_hiragana_only(convert_word_to_hiragana(yomikata if yomikata else word))
